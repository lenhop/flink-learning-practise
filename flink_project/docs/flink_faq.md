...
## 10. JAR 文件是否需要放入 Docker Flink 容器中

**取决于 PyFlink 的运行模式**：

### 远程模式（连接 Docker Flink 集群）

**需要** JAR 文件在容器中，通过 Docker volume 挂载实现。

**本机 Docker 部署**：虽然 Docker 容器运行在本机，但从 PyFlink 脚本角度看仍属于远程模式。PyFlink 客户端在容器外，Flink 运行时在容器内，需要通过 `localhost:6123` 连接。JAR 文件必须挂载到容器中。

**配置方式**（docker-compose.yml）：
```yaml
volumes:
  - ../custom_jars:/opt/flink/lib/custom
```

**PyFlink 代码**：不需要调用 `env.add_jars()`，Flink 会自动加载容器中 `/opt/flink/lib/custom/` 目录下的所有 JAR 文件。

```python
from pyflink.common import Configuration

cfg_obj = Configuration()
cfg_obj.set_string("jobmanager.address", "localhost")
cfg_obj.set_string("jobmanager.rpc.port", "6123")
# 不需要 add_jars()，JAR 文件已在容器中
env = StreamExecutionEnvironment.get_execution_environment(cfg_obj)
```

### 本地模式（本地运行，不连接集群）

**不需要** JAR 文件在容器中，使用本地文件系统路径。

**PyFlink 代码**：需要通过 `pipeline.classpaths` 或 `env.add_jars()` 指定本地路径。

```python
# 方式 1：使用 pipeline.classpaths
cfg_obj = Configuration()
cfg_obj.set_string("pipeline.classpaths", "file:///path/to/flink-connector-jdbc.jar")

# 方式 2：使用 env.add_jars()
env = StreamExecutionEnvironment.get_execution_environment()
env.add_jars("file:///path/to/flink-connector-jdbc.jar")
```
...
**验证容器中的 JAR 文件**：
```bash
docker exec flink-jobmanager-mysql ls -la /opt/flink/lib/custom/
```

**添加新的 JAR 文件**：
1. 复制到主机挂载目录：`cp new-connector.jar /path/to/custom_jars/`
2. 重启容器：`docker-compose restart`
3. 验证：`docker exec flink-jobmanager-mysql ls -la /opt/flink/lib/custom/`
